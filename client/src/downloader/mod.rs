...
impl Downloader {
    ...
    pub async fn check_existing_data(
        ...
        let file_len = file.metadata().await?.len();
        let mut piece_status = self.piece_status.lock().await;

        for i in 0..piece_count {
            let offset = i as u64 * self.torrent.piece_length;
            let len = if i == piece_count - 1 {
                let rem = self.total_length % self.torrent.piece_length;
                if rem == 0 {
            ...
            if offset + len <= file_len {
                ...
                if file.read_exact(&mut buf).await.is_ok() {
                    ...
                    if hash.as_slice() == &self.torrent.pieces[i] {
    ...
    pub async fn run(&mut self) {
        ...
        tracker_urls.push(self.torrent.announce.clone());
        if let Some(list) = &self.torrent.announce_list {
            for tier in list {
                for url in tier {
                    if *url != self.torrent.announce {
        ...
        let downloaded_total = Arc::new(Mutex::new(self.downloaded_bytes));
            ...
            tokio::select! {
                res = peer_rx.recv() => {
                    if let Some(peer_addr) = res {
                        ...
                        let piece_status = self.piece_status.clone();
